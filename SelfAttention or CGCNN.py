...
class ConvLayer(nn.Module):
    ...
    def __init__(self, atom_fea_len, nbr_fea_len,attn=True):
        ...
        self.attn = SelfAttention(input_size=2 * self.atom_fea_len + self.nbr_fea_len,
                                      attention_size=self.atom_fea_len, output_size=self.atom_fea_len)
    ...
    def forward(self, atom_in_fea, nbr_fea, nbr_fea_idx):
        ...
        if self.attn_type:
            ...
            nbr_filter, nbr_core = total_gated_fea.chunk(2, dim=2)  # 论文中的Eq. 5 简单的注意力机制 (论文描述为element-wise multiplication)  可以换成Self-attention
            nbr_filter = self.sigmoid(nbr_filter)
            nbr_core = self.softplus1(nbr_core)
            nbr_sumed = torch.sum(nbr_filter * nbr_core, dim=1)
            nbr_sumed = self.bn2(nbr_sumed)

        out = self.softplus2(atom_in_fea + nbr_sumed)  ## atom_in_fea作为残差学习更好  nbr_sumed就是GCN后的邻居特征
        return out
